...
interface DashboardProps {
  ...
  onNavigate: (tab: 'dashboard' | 'spotmap' | 'patients' | 'contacts' | 'line-gas') => void;
      ...
      <div className="bg-gradient-to-r from-emerald-800 via-teal-800 to-slate-900 text-white rounded-2xl p-5 shadow-sm border border-emerald-700/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight">
            สรุปสถานการณ์วัณโรค อ.โพนนาแก้ว ปีงบประมาณ 2569
          </h2>
          <p className="text-slate-300 text-xs sm:text-sm max-w-2xl">
            ติดตามการกินยารายวันแบบ DOTS, คัดกรองผู้สัมผัสเสี่ยงสูงร่วมบ้าน และระบบแจ้งเตือนเจ้าหน้าที่ผ่าน LINE Notify
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 self-stretch md:self-auto">
          ...
          >
            ...
            <span>บันทึกคัดกรองผู้สัมผัส</span>
          </button>
        </div>
      </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-600 font-semibold uppercase tracking-wider border-y border-slate-200">
                <tr>
                  ...
                  <th className="py-3 px-3 text-center">ผู้สัมผัสร่วมบ้าน</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {subdistricts.map(sub => {
                  ...
                  const subContacts = contacts.filter(c => c.subdistrict === sub.name).length;
